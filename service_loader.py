...
class ResponseRule:
    ...
    def _compile(self, encoding: str) -> None:
        if self._compiled_regex is None:
            self._compiled_regex = re.compile(self.match.encode(encoding), re.IGNORECASE)
...
def _service_from_mapping(service_map: Dict[str, Any], default_bind_ip: Optional[str]) -> ServiceDefinition:
    ...
    responses = [
        ...
        for resp in service_map.get("responses", [])
    ]
    return ServiceDefinition(
        name=service_map.get("name", f"service_{port}"),
        protocol=service_map.get("protocol", "tcp"),
        port=int(port),
        bind_ip=bind_ip,
    ...
    for (_, _, filenames) in os.walk(handler_dir):
        for filename in filenames:
            ...
            if protocol == "http" and cfp.has_section("responses"):
                responses = [
                    ResponseRule(match=pair[0], body=pair[1]) for pair in cfp.items("responses")
                ]
